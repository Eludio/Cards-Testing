...
int calculateMax(int value1, int value2){
  if (value1 >= value2){
    return value1;
  }
  else{
...
int isInRange(int coord, int offset, int size) {
  ...
  if(coord >= offset && coord < endpoint){
...
int isAtBorder(int coord, int offset, int size) {
  int border = offset + size;
  if(coord == offset || coord == border){
...
void squares(int size1, int x_offset, int y_offset, int size2) {
  ...
  int w;
  w = calculateMax(size1, x_offset + size2);
  //compute the max of size1 and (y_offset + size2) Call this h
  int h;
  h = calculateMax(size1, y_offset + size2);
